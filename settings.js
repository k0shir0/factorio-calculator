...
// This site ships base vanilla Factorio 2.0 only. Other data sets (1.1,
// expensive, Space Age) and the version switcher have been intentionally
// removed; see the hidden #data_set select in index.html.
export let MODIFICATIONS = new Map([
    ["2-0-55", new Modification("Vanilla 2.0.55", "vanilla-2.0.55.json", false)],
])

let DEFAULT_MODIFICATION = "2-0-55"

// Map any older/legacy data-set keys from saved URLs onto the single shipped
// data set so old links still resolve instead of erroring.
var modUpdates = new Map([
    ...
    ["1-1-110", "2-0-55"],
    ["1-1-110x", "2-0-55"],
    ["space-age-2-0-10", "2-0-55"],
    ["space-age-2-0-11", "2-0-55"],
    ["space-age-2-0-55", "2-0-55"],
])
...
function renderTab(settings) {
    ...
    // A ?bp=<slug> query param is a shareable deep link into a Blueprints
    // article; it always wins so the page opens on the Blueprints tab.
    if (new URLSearchParams(window.location.search).has("bp")) {
        tabName = "blueprints"
    }
    clickTab(tabName)
}
